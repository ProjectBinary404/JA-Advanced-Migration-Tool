describe("Joomla Installation Wizard", () => {
  it("should complete Joomla installation", () => {
    ...
    // Close guided tour if it appears
    cy.get("body").then(($body) => {
      // Check for guided tour modal/overlay and close it
      if ($body.find('[data-bs-dismiss="modal"]').length > 0) {
        cy.get('[data-bs-dismiss="modal"]').first().click();
      }
      // Alternative: Check for guided tour close button
      if ($body.find('.btn-close, .close, [aria-label="Close"]').length > 0) {
        cy.get('.btn-close, .close, [aria-label="Close"]').first().click();
      }
      // Check for guided tour skip button
      if (
        $body.find('button:contains("Skip"), a:contains("Skip")').length > 0
      ) {
        cy.get('button:contains("Skip"), a:contains("Skip")').first().click();
      }
      // Check for guided tour dismiss button
      if (
        $body.find('button:contains("Dismiss"), a:contains("Dismiss")').length >
        0
      ) {
        cy.get('button:contains("Dismiss"), a:contains("Dismiss")')
          .first()
          .click();
      }
    });
